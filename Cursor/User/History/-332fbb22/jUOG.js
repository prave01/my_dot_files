...
function startListening() {
    showError("Voice search is not implemented yet.");
}
...
function renderTopItems(topItems) {
    const el = document.getElementById('itemConsumptionChart');
    if (!el) return;
    ...
    if (window.Chart) {
        el.innerHTML = '<canvas id="topItemsChart"></canvas>';
...
function renderUserActivity(userActivity) {
    ...
    if (!userActivity || userActivity.length === 0) {
        el.innerHTML = '<p>No user activity data.</p>';
        return;
    }
    // If Chart.js is available, use it
    if (window.Chart) {
        el.innerHTML = '<canvas id="userActivityChart"></canvas>';
        ...
        el.innerHTML = userActivity.map(u => `
            <div class="top-item"><span class="item-name">${u._id}</span><span class="item-count">${u.totalTransactions}</span></div>
        `).join('');
    }
}
