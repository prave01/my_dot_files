...
function addItem() {
  const itemName = document.getElementById("newItem").value.trim().toLowerCase();
  ...
  if (!itemName || !make || !model || !specification || !rack || !bin || !qty) {
  ...
  // Add transaction
  addTransaction('new_item', itemName, qty, currentUser.username, {
    make: make,
    model: model,
    specification: specification,
    rack: rack,
