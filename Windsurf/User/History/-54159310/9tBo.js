...
function deleteUser(username) {
  ...
  // Refresh UI if in user management
  if (typeof populateUserSelect === 'function') {
    populateUserSelect();
  }
...
// deleteUser function is now defined above with enhanced functionality
...
function openUpdate() {
  if (!currentUser || currentUser.role !== 'admin') {
    alert('❌ Access denied! Only admins can update items.');
    return;
  }
  toggleMenu();
  goBack();
...
// openHistory function is now defined above with proper access control

// openUserManagement function is now defined above with proper access control

// Search functionality is implemented in the main searchItem function below
...
// openUpdate function is now defined above with proper access control
...
// openUpdateQuantity function is now defined above with proper access control
