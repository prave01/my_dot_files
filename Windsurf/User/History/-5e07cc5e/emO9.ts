...
import { loginSchema, insertUserSchema, insertInventorySchema, updateQuantitySchema, changePasswordSchema, Transaction } from "@shared/schema.js";
...
export async function registerRoutes(app: Express): Promise<void> {
  ...
  app.get("/auth/validate", authenticateToken, async (req: AuthRequest, res: Response) => {
  ...
  app.put("/users/:id/password", authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
    try {
  ...
  // Inventory routes
  app.get("/inventory", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
  ...
  app.get("/inventory/search", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
  ...
  app.post("/inventory", authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
    try {
  ...
  app.patch("/inventory/:id/quantity", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      const itemId = req.params.id;
      const updateData = updateQuantitySchema.parse(req.body);

      const updatedItem = await storage.updateInventoryItem(itemId, updateData.quantity, updateData.type);
      ...
      await storage.createTransaction({
        ...
        userId: parseInt(req.user!.id),
        username: req.user!.username,
        details: { action: "update_quantity", item: updatedItem }
      });
  ...
  app.delete("/inventory/:id", authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
    try {
      const itemId = req.params.id;

      const item = await storage.getInventoryById(itemId);
      if (!item) {
      ...
      await storage.createTransaction({
        ...
        userId: parseInt(req.user!.id),
        username: req.user!.username,
        details: { action: "delete_item", item }
      });
  ...
  app.get("/transactions", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const transactions = await storage.getTransactions({ limit, itemId, userId });
  ...
  app.get("/analytics/stats", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
  ...
  app.get("/analytics/most-used", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
  ...
  app.get("/analytics/user-activity", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      const limit = req.query.limit ? parseInt(req.query.limit as string) : 10;
  ...
  app.delete("/transactions", authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
    try {
  ...
  // CSV Export route
  app.get("/inventory/export", authenticateToken, async (req: AuthRequest, res) => {
    try {
      const items = await storage.getAllInventory();

      if (items.length === 0) {
        return res.status(404).send("No inventory to export.");
      }

      const csvHeader = "ID,Name,Quantity,Category,Supplier,Date Added\n";
      const csvRows = items.map(item =>
        `${item._id || item.id},"${item.name}",${item.quantity},"${item.category}","${item.supplier || ''}","${new Date(item.createdAt).toISOString()}"`
      ).join("\n");
