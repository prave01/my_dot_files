...
export async function registerRoutes(app: Express): Promise<void> {
  ...
  app.post("/auth/login", async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!user || !(await storage.validateUserPassword(user, password))) {
  ...
  app.get("/api/inventory", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      res.json(items.map(i => ({ ...i, id: i._id || i.id })));
    } catch (error) {
  ...
  app.post("/api/inventory", authenticateToken, requireAdmin, async (req: AuthRequest, res: Response) => {
    try {
      ...
      res.status(201).json({ ...item, id: item._id || item.id });
    } catch (error) {
  ...
  app.patch("/api/inventory/:id/quantity", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const updateData: import("@shared/schema.js").UpdateQuantityData = updateQuantitySchema.parse(req.body);
      ...
      res.json({ ...updatedItem, id: updatedItem._id || updatedItem.id });
    } catch (error) {
  ...
  app.get("/api/transactions", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      res.json(transactions.map(t => ({ ...t, id: t._id || t.id })));
    } catch (error) {
  ...
  app.get("/api/analytics/user-activity", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      const limit = req.query.limit ? parseInt(req.query.limit as string) : 10;
      const activity = await storage.getUserActivity(limit);
      res.json(activity);
    } catch (error) {
  ...
  app.get("/api/inventory/export", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      const items = await storage.getAllInventory();
      ...
      const csvContent = [
        headers.join(","),
        ...items.map(i => [
          `"${i.name}"`, `"${i.make || ''}"`, `"${i.model || ''}"`, `"${i.specification || ''}"`,
          `"${i.rack || ''}"`, `"${i.bin || ''}"`, i.quantity
        ].join(","))
      ].join("\n");
