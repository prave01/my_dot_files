...
// Define local types since we can't import from @/types
interface Member {
  id: number;
  name: string;
  email: string;
...
export default function Dashboard() {
  ...
  const entries = ((recentEntries || []) as any[]).map(entry => ({
    ...entry,
    timestamp: entry.timestamp instanceof Date ? entry.timestamp : new Date(entry.timestamp)
  })) as EntryWithMember[];
  const members = ((membersData?.members || []) as any[]).map(member => ({
    ...member,
  ...
  const filteredEntries = entries.filter((entry) => {
    ...
    return (
      entry.member?.name?.toLowerCase().includes(searchLower) ||
      entry.member?.email?.toLowerCase().includes(searchLower)
    );
  });
              ...
              <div className="ml-4">
                ...
                {statsLoading ? (
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {dashboardStats?.currentEntries ?? 0}
                  </p>
                )}
              </div>
            </div>
                  ...
                  <TableRow key={entry.id}>
                    <TableCell>
                      <div className="flex items-center">
                        ...
                        <div>
                          <div className="font-medium">{entry.member?.name || 'Unknown Member'}</div>
                          <div className="text-sm text-gray-500">{entry.member?.email || ''}</div>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="text-gray-600">{entry.member?.phone || 'N/A'}</TableCell>
                    <TableCell>
                      <Badge 
